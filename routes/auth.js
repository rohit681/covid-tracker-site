...
const covid = require("../models/user");
...
router.post("/api/auth", async (req, res) => {
  ...
  try {
    const isAlreadyExist = await covid.findOne({ email });
    if (isAlreadyExist) {
      return res
        .status(422)
    ...
    user = await covid.create({ name, email, password });
...
router.post("/api/login", async (req, res) => {
  try {
    ...
    const isUserExist = await covid.findOne({
      authToken: null,
      email: email,
    });
    if (!isUserExist) {
...
router.post("/api/finduser", finduser, async (req, res) => {
  try {
    userId = req.user.id;
    const findUser = await covid.findById(userId).select("-password");
    res.send(findUser);
  } catch (err) {
    console.log(err);
  }
